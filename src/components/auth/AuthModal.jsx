 ...
 */
function AuthModal({ open, onClose, role = 'patient', initialMode = 'login', lockMode = false }) {
  ...
  const handleSignup = (e) => {
    ...
    if (role === 'compounder' && !form.staffId?.trim()) err.staffId = 'Required'
    if (!form.password?.trim()) err.password = 'Required'
    ...
    >
      ...
      >
        ...
        <div className="flex w-full flex-col p-7 md:w-[58%]">
          ...
          {/* Tabs */}
          {!lockMode && <div className="mb-4 flex gap-1 rounded-xl bg-slate-100 p-1">
            {mode === 'signup' ? (
              <>
          ...
          {mode === 'signup' && (
            <form onSubmit={handleSignup} className="space-y-3">
              ...
              {role === 'patient' ? (
                ...
                <TextInput icon={Stethoscope} placeholder="Clinic / Staff ID" value={form.staffId || ''} onChange={set('staffId')} error={errors.staffId} />
              )}
