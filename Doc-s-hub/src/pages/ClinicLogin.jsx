...
import { DUMMY } from '../data/authData.js'
...
function ClinicLogin() {
  ...
  const handleLogin = (e) => {
    ...
    const ok = DUMMY.clinic.some((c) => c.email === form.idf.trim() && c.password === form.password)
    if (ok) {
      setBanner({ type: 'success', msg: 'Login successful! Opening your clinic dashboard…' })
      login('clinic', { email: form.idf })
      setTimeout(() => navigate('/clinic-dashboard'), 850)
    } else {
      setBanner({ type: 'error', msg: 'Invalid credentials. Use the demo credentials below.' })
    }
  }
      ...
      <main className="mx-auto grid max-w-[1180px] gap-6 px-6 py-8 lg:grid-cols-[1.45fr_1fr] lg:items-start">
        ...
        <section className="rounded-3xl border border-slate-100 bg-white p-7 shadow-card lg:sticky lg:top-8">
          ...
          <form onSubmit={handleLogin} className="mt-4 space-y-3">
            ...
            <button className="flex w-full items-center justify-center gap-2 rounded-xl bg-brand-blue py-3 text-sm font-semibold text-white transition-colors hover:bg-brand-blueDark">
          ...
          <div className="mt-5 rounded-2xl border border-blue-100 bg-brand-blueLight p-4">
            ...
            <p className="mt-1 text-xs text-slate-500">Use the credentials below to explore the clinic dashboard.</p>
            <div className="mt-2 space-y-1 text-[13px] text-brand-navy">
              <p className="flex items-center gap-2"><Mail className="h-3.5 w-3.5 text-slate-400" /> clinic@doctormitra.com</p>
              <p className="flex items-center gap-2"><Lock className="h-3.5 w-3.5 text-slate-400" /> Clinic@123</p>
            </div>
          </div>
        </section>
      </main>
