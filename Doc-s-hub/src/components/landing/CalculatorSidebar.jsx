  ...
  return (
    <aside className="flex h-full w-[280px] shrink-0 flex-col gap-4 overflow-hidden py-5 pl-5 pr-1">
      <div className="flex min-h-0 flex-1 flex-col rounded-2xl border border-slate-100 bg-white px-4 py-4 shadow-card">
        ...
        <ul className="flex min-h-0 flex-1 flex-col justify-between py-1">
          {HEALTH_CALCULATORS.map(({ label, short, icon: Icon }) => (
            <li key={label}>
              <button className="group flex w-full items-center gap-3 rounded-xl px-2 py-2 text-left transition-colors hover:bg-brand-greenLight">
      ...
      <div className="shrink-0 rounded-2xl border border-green-100 bg-brand-greenLight p-4">
        ...
        <button className="mt-2 flex items-center gap-1 text-[13.5px] font-semibold text-brand-green hover:gap-1.5">
          Explore Calculators
